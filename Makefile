...
test-host:
	luajit tests/ja4_helpers_test.lua && luajit tests/hygiene_test.lua && luajit tests/reputation_test.lua

test-docker:
	docker run --rm -v $(PWD):/work -w /work $(OPENRESTY_IMAGE) \
		sh -c "luajit tests/ja4_helpers_test.lua && luajit tests/hygiene_test.lua && luajit tests/reputation_test.lua"
