# Design #2 / Phase 1 — a positive catalog of browser fingerprints (D2) plus hash mismatch
...
coarse `cipher_cnt`, plus a sharpened purity gate from #1 (D12).
...
3. **The edge — strengthen `tls_fp_suspicious_ciphers`**
   ...
   **Why the full fingerprint rather than `hash_b` (caught in review by Codex):**
   ...
   Family/version comparison is Phase 2 (D3). It is **soft-only** (a challenge or a
