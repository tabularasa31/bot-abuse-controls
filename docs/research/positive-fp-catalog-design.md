...
### Доступ фермы к `/__fp` под захардненным эджем — ОТЛОЖЕНО, решить при сборке фермы

> Контекст возник при хардненге эджа (feature/edge-deny-nontenant): эдж переводится
> в режим «публичный `:443` обслуживает только тенантов; no-SNI / IP-литерал /
> не-тенантская SNI → reject (TLS) / 444 (HTTP); эксплуатационные ручки уходят на
> приватный listener». В этой постуре `/__fp` БОЛЬШЕ НЕ публично-открыт, и наивное
> «ферма гонит браузер на `https://<stand>/__fp`» (выше по тексту) перестает работать.
> Ниже зафиксированы варианты; финальный выбор — когда будем делать ферму.

Жесткие условия (определяют дизайн):

- **Снять надо НАСТОЯЩИЙ браузерный JA4.** Любой посредник, ТЕРМИНИРУЮЩИЙ TLS
  (https-прокси, MITM), снимет fp посредника, а не браузера → обессмысливает ферму.
  Любой шлюз на пути обязан быть L4 (TCP/NAT passthrough), TLS — end-to-end браузер↔эдж.
- **`/__fp` обходит каскад и всегда 200** (см. [probe.lua](../../infra/demo-stand/lua/probe.lua)) —
  это остается, проба низкочувствительна (отдает свой же fp), но не должна быть
  спуфабельной дырой в «только-тенант» постуре.
- **Порт/хост — не контроль.** Скан найдет выделенный `:8443` за секунды; обскурити
  не защищает. Контроль — либо firewall по source-IP (L4, до TCP), либо крипто (ключ/серт).
- **Ферма GitHub-hosted ⇒ фиксированного egress-IP НЕТ.** Диапазоны GitHub Actions
  огромны и общие → firewall-allowlist по ним невозможен. Второй (плавающий) IP на
  ТОЙ ЖЕ VM, что и эдж, бастионом быть НЕ может (не создает фиксированный источник на
  пути фермы; SNAT должен происходить НЕ на эдже). Такой IP годится лишь для изоляции
  поверхности пробы (bind пробы на отдельный IP:порт, свой серт/SNI), но не как
  access-control.

Варианты (оба сохраняют fingerprint — не ретерминируют TLS):

- *Решение A — mTLS на пробе (без новой инфры).* Выделенный пробный server
  (отдельный IP:порт, свой `server_name`/cert) с `ssl_verify_client on` + приватный
  CA фермы; клиентский серт лежит в GitHub Actions secrets, Playwright предъявляет его
  (`clientCertificates`). Без серта — TLS-handshake падает на верификации, ДО `/__fp`.
  JA4 сохраняется (клиентский серт уходит ПОСЛЕ ClientHello, а JA4 считается из
  ClientHello). **Проверить при сборке:** включение клиентского серта в браузере не
  добавляет лишних расширений в ClientHello (иначе снятый JA4 разойдется с продом, где
  mTLS нет) — разовая валидация «hello с mTLS == hello без mTLS». Остаточно: флуд
  handshake'ами до верификации серта дешевле полного, но не L4-бесплатен → rate-limit.
- *Решение B — WireGuard-бастион (отдельная статическая VM).* CI поднимает WG-туннель
  (ключ из secrets), браузер ходит через туннель, бастион SNAT'ит в свой статический IP;
  эдж firewall: проба DROP для всех, кроме IP бастиона. WG это L3 → TLS end-to-end,
  JA4 идеальной точности, порт закрыт на сетевом уровне. Цена — одна мелкая VM.

Зависимости для выбора: (1) умеет ли ферма (Playwright/Puppeteer/Selenium) предъявлять
клиентский серт; (2) подтверждается ли fidelity ClientHello под mTLS; (3) готовы ли поднять
мелкую бастион-VM. Дефолт-кандидат: Решение A (без новой инфры), Решение B как fallback.
