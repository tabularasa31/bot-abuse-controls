...
> каталоги), B9 (log receiver), Channel C (B5/B6), D14/D15 (subnet-юнит), D12 (solve-rate).
...
```
1. Атака на тенанта A → эдж видит плохие fp/subnet (реактивные сигналы D15)
...
5. У каждого тенанта — ПРЕД-ВЗВОД для этих fp/subnet:
     ...
     • тенант ПОД атакой → может эскалировать до DROP (локальная логика D15)
     • тенант НЕ под атакой → только challenge-first, НИКОГДА не авто-block
6. Записи сами истекают по TTL
     повторяется/подтверждается → промоут в МЕДЛЕННЫЙ tier обычным PR-gated пайплайном (D1/D14)
```
...
- к DROP приводит **только** если тенант сам под атакой (его attack_mode) И локальные сигналы D15
  согласны.
...
- те же purity/human_share/DC-only гейты, что в D1/D14, но строже;
...
- **D14:** `subnet_reputation` закладывать **глобальным** артефактом (не пер-хост), иначе
  кросс-тенантность потом ломать. (Комментарий добавлен в D14.)
...
- D14 (subnet-репутация, глобальная), D15 (реактивные сигналы атаки), D12 (solve-rate).
