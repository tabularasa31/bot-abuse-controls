...
| **L5 verification** (`verification.lua`, `challenge*.lua`) | JS challenge, Strictness, `attack_mode` (C7) | **L7 anti-DDoS**: the challenge as a filter under load |
...
3. **The delivery and observability infrastructure is reusable.** Channel C
   ...
   (`bac_log.lua`), the metrics and the kill switch (A12) all get reused for both WAF
...
| Threat | What the edge can do | Fit |
|---|---|---|
| API scraping / abuse | partly covered by `rate_api` (A7) and `rate_scan_urls`; new: per-API-key quotas | ★★★ |
