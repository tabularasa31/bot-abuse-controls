...
уже оформленными тикетами (G-серия + один сигнал из слоя D):

| Идея | Где уже живёт (ClickUp / research) |
|---|---|
| Авто-взвод `attack_mode` по «плохому» трафику (не голому объёму), гистерезис, precedence ручного над авто, per-host флаг `auto_attack_mode` | **[G3]** `86ext6yuq` (backlog, дизайн готов) — детект по bot-rate/solve_rate/origin-latency относительно базлайна хоста |
| Subnet/ASN-репутация DC-пулов (soft, аналитика) | **[G1]** `86ext6yn6` (backlog) + [subnet-unit-design.md](subnet-unit-design.md) |
| Transient subnet challenge→drop под атакой | **[G2]** `86ext6ytk` (backlog) |
| Быстрый hot-list атакующих (пред-взвод эджей при пивоте ботнета) | **[G4]** `86ext718e` (backlog) + [cross-tenant-threat-intel-design.md](cross-tenant-threat-intel-design.md) |
| solve-rate как сигнал бота под флудом (сам тикет — **слой D**, детектор; G-серия его переиспользует) | **[D12]** `86ext5daf` (review) + [challenge-solve-rate-design.md](challenge-solve-rate-design.md) |
...
нового скоупа нет — он закрывается завершением G1–G4. Не плодим дубль-тикеты.
...
сетевых/инфра-админов прод-эджа, прод-доступа к которой у нас нет.
...
- **исследовательский ADR/дизайн** по eBPF/XDP-дропу как будущей фазе — без реализации
  на проде.
- НЕ выдумывать интеграцию с salt/Puppet прод-эджа (CLAUDE.md §«Чего НЕ делать»). Если
...
- **DDoS L7 rate-based — уже в бэклоге (слой G):** [G1] `86ext6yn6` (subnet-репутация),
...
- Не лезем в прод-сеть (salt/Puppet/eBPF на боевых эджах) — нет доступа, не наша фаза.
