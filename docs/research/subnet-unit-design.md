...
- A mature **per-fingerprint pipeline** : score, gates, staging, promote.
...
2. Escalate challenge → DROP only once the challenge stops paying for itself:
     ...
     (b) the pool's solve_rate ≈ 0 WITH issued >= MIN_SUBNET_CHALLENGES (the the solve-rate work guard):
...
false DROP. This is the same asymmetric guard as in the solve-rate work (`MIN_CHALLENGE_ISSUED`).
...
`antibot_challenge_issued_total`  times its cost against the edge budget, plus
`cascade_ms` (BAC_LOG timing); and `solve_rate≈0` for the pool  — the formal
...
- the host's `solve_rate` falling through the floor ;
...
- **#3 Phase 1** — subnet reputation (soft, analytics): the datacenter gate,
...
- **#3 Phase 2** — transient subnet challenge→drop during an attack:
...
- **automatic attack mode** — adaptive attack detection (bad traffic plus
