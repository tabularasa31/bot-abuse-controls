...
> Аккумулятор `seen-fps.json` ротируется (D7, `rotate-state.py`), но fp **из каталога
> блок-листа освобождены** от архивации/дропа — иначе молчащий enforced-fp уехал бы в архив
> и выпал из этой проверки (она читает только активный `seen-fps.json`). Поэтому сигнал
> auto-demote по блок-листным fp сохраняется. Детали ротации:
