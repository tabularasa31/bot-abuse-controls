...
| Analytics core — scoring, evidence, the blocklist lifecycle | `analytics-*` | partly built: scoring in `infra/demo-stand/scripts/analyze.py`, promotion tooling in `scripts/`; the rest is design |
| Product frame — which adjacent categories are worth taking on | `product-scope.md` | direction, not a commitment |

Each family follows the same shape: the spec itself, a rules reference, an
entities reference and — except for analytics — config templates.
