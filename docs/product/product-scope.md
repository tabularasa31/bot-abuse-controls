...
2. **Всё, что ниже — пакетный/волюметрический флуд (SYN, UDP, амплификация) — НЕ наше
   ...
   Это сетевой уровень (eBPF/XDP, anycast-scrubbing, BGP) — зона инфры/prod-edge.
...
- **Волюметрика L3/L4** — только в связке с сетевым уровнем (prod-edge/инфра).
...
prod-edge/инфра-админов, прод-доступа нет). Не выдумывать как сделанное; понадобится прод-сеть —
