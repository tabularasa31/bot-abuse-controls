...
> ⚠️ Это HTTP/2 DoS-mitigation (E4), а не HTTP/2 fingerprint (R2). Frame-уровень
> не доходит до HTTP-семантики, каскад его не видит вовсе. fp (R2) — отдельная задача
...
> IP / /24 питают репутацию (G1) и через неё edge-ACL feed.
