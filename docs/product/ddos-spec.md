...
Этот документ описывает **целевое поведение DDoS-защиты** как отдельный слой поверх
каскада ([vision.md](vision.md)): спецификация «бери и делай», по которой строится
реализация. Каскад из vision принимает решение по каждому *запросу*; DDoS-слой имеет
дело с тем, что лежит **до** и **рядом** с запросом: частотой и формой нагрузки,
поведением соединений и протокола, объемом на сетевом уровне.
...
- `client_header_timeout` / `client_body_timeout` / `send_timeout` — срезать до
  10–15s: рвет соединения, не завершающие заголовки/тело/чтение (отказ `408`).
...
Lua-хук в log-фазе читает `$status` / `$request_time` / `$connection_requests`;
таймаут (`408`) и отказ `limit_conn` (`503`) → лог-событие с тегом `slow_client` /
`conn_flood` → дашборд + счетчик по IP / /24. Ограничение: видно только то, что nginx
отдает в log-фазе; сам процесс цежения соединения в реальном времени не виден.
