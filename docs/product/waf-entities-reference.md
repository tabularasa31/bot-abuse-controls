...
**Статус:** проектный контракт (целевое поведение); движок (build-vs-buy) не выбран — открыт в ADR-007. Имена стадии/каталога/флагов/полей контрактны и держатся независимо от выбора движка.
...
| **Движок `waf.lua`**       | Реализация стадии: пре-процессор нормализации + матч сигнатур. Путь реализации (своя Lua / Coraza+CRS / гибрид) выбирается в ADR-007. До ADR в код не коммитимся.                                                                     |
| **Каталог `waf_rules`**    | Набор сигнатур атак, доставляемый на proxy через Channel C ровно как `tls_fp_blocklist`: git — единственный источник истины (ADR-006), PR-only + CODEOWNERS, staged rollout, `git revert`. Формат правил определяется ADR-007.        |
| **Сигнатура**              | Одно правило-паттерн внутри `waf_rules` под конкретный класс атаки или CVE. Имеет id, класс, критичность, статус (staging/active). Внутренности (regex/seclang) абстрактны до ADR-007.                                                |
...
| **Per-host WAF-профиль**   | Расширение policy домена: `waf_enabled`, `waf_paranoia`, `waf_disabled_rules`. Редактируется через B10 Policy API (PATCH). Независим от `mode`/`strictness`.                                                                          |
...
| ----------- | --------------------------------------------------------------------------------------------------- | ----------------- | ----------------------------------------- | --------------------- |
| `waf_rules` | Сигнатуры атак (SQLi, XSS, path-traversal, command-injection, SSRF/LFI) + virtual-patch правила; каждая запись с id, классом, критичностью, статусом | Продакт/команда через PR (CODEOWNERS) | Channel C (git → proxy), как `tls_fp_blocklist` (ADR-006) | `tls_fp_blocklist` — блокирующий каталог через PR + staged rollout |
...
Редактируются через B10 Policy API (PATCH). Не зависят от `mode`/`strictness`: `mode` управляет shadow/active исполнением, WAF-профиль — охватом инспекции.
