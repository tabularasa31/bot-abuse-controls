...
Идея: посмотреть на все запросы под этой сигнатурой и прикинуть, какая их часть похожа на
живого человека в настоящем браузере. Если таких заметная доля — блокировать нельзя.

- Берем все запросы под сигнатурой за окно (например, 1000).
- Каждый запрос помечаем «похож на настоящий браузер», только если выполнено всё сразу:
  1. в `User-Agent` назван настоящий браузер (Chrome/Firefox/Safari…), а не curl/python/бот;
  2. сам TLS-отпечаток браузерный — число шифров как у браузеров (15/16/20), а не как у
  инструмента;
  3. отпечаток не совпадает с известным инструментом из словаря (curl/python/headless).
- Считаем долю помеченных: например, 30 из 1000 = 3%.
- Пройден, если доля ≤ 5% (живых людей под сигнатурой почти нет — корзина ботовая); если
выше — вето (там есть реальные пользователи).

(Условия 2 и 3 — это свойства самого отпечатка, одинаковые для всех его запросов; от запроса
к запросу меняется только `User-Agent`. Поэтому доля по сути показывает, как часто под этим
рукопожатием приходили с настоящим браузерным UA.)
