...
2. При достаточном числе выданных посчитать долю решенных. 3. Пройден, если доля низкая (челленджи почти не решаются — так ведет себя бот). Высокая доля — вето: челленджи проходят, значит под отпечатком живые люди. Низкая доля вдобавок ускоряет переход `наблюдение → блокировка`.
