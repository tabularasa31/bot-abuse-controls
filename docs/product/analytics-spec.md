...
| позитивный каталог браузеров (`tls_fp_browser_profiles`) | белый список «известно-хороших» отпечатков браузеров                                                          | гейт «бот-онли» (genuine-browser), гейт known-good, сигнал расхождения версий |
...
Важно: гейты живут в решении автомата — они определяют, предложит ли автомат блокировку
сам. На сам каталог гейтов нет: человек-в-петле правит блок-лист своим PR и формально может
завести любой fp — гейт его не остановит. То, что непройденный гейт перебивает любую
оценку, — общее свойство всех гейтов (см. выше).
...
- **Вето** (бот-онли, доверенные, known-good, решение челленджей) — про безопасность: за ними живые пользователи и легитимный трафик. Пройти мимо вето вручную = сознательно заблокировать невиновных. Технически человека ничто не держит, но это красная черта, а не рядовая операция.
...
Каждый гейт — простой детерминированный расчет по событиям сигнатуры за окно анализа (те
самые последние 7 дней из Loki, что воркер читает за прогон). Окно скользящее и
пересчитывается каждый суточный прогон заново: сегодня — по 7 дням до сегодня, завтра — по 7
дням до завтра. Исключение — гейт «объем»: он смотрит не на окно, а на счетчик за все время
из накопителя (7 дней мало, чтобы судить об объеме). По шагам: 

**бот-онли** (вето).

- Вход: все события сигнатуры за окно.
- Считаем: событие = «как настоящий браузер», если одновременно UA браузерный, число шифров
из браузерного набора, хеши не из словаря инструментов. `доля = такие / всего`.
- Пройден, если `доля ≤ 5%`; иначе вето.

**объем.**

- Вход: накопленный за все время счетчик событий по сигнатуре (из накопителя) и ее IP за окно.
- Пройден, если `событий ≥ 20` И `IP ≥ 1`; иначе придержать.

**доверенные** (вето).

- Вход: вердикты и `rule` всех событий сигнатуры, ее IP.
- Считаем: был ли хоть один «пропуск как доверенный» — `verdict=allow` по verified-bot / IP в
whitelist / валидной clearance-cookie; есть ли IP сигнатуры в whitelist.
- Пройден, если ни того ни другого нет; иначе вето.

**дубль.**

- Пройден, если сигнатуры еще нет в блок-листе; иначе ничего не добавляем (идемпотентность).

**обоснованность.**

- Считаем флаги: `маскировка` (браузерный UA на инструментальном fp), `разведка` (recon-URI под
fp), `общий честный инструмент` (fp из словаря инструментов И не маскировка).
- `обоснованность = маскировка ИЛИ (разведка И НЕ общий честный инструмент)`.
- Пройден, если обоснованность истинна; иначе автоблока нет (кейс для адресного UA/IP, решает
человек).

**known-good** (вето).

- Пройден, если fp НЕ совпал с позитивным каталогом браузеров; совпал → вето (настоящий браузер).

**решение челленджей** (вето).

- Вход: счетчики выданных и решенных challenge по fp (только с `active`-хостов и attack-off —
см. оговорку ниже).
- Считаем долю решенных при достаточном числе выданных.
- Пройден, если доля низкая (челленджи почти не решаются); высокая доля → вето (под fp люди).
Низкая доля вдобавок ускоряет переход к блокировке.
...
Важно сразу понять, откуда под сигнатурой живые люди. Отпечаток задаётся TLS-стеком, а
не личностью: если бот ходит с тем же стеком, что и какой-то реальный (возможно, редкий
или незнакомый нам) браузер, под одним fp оказываются и бот, и живые пользователи этого
браузера. Заблокируешь fp — заденешь людей. Речь именно про fp настоящего браузера,
переиспользованный ботом, — не про инструментальный fp (там людей нет, см. Риск 2).

Чтобы это поймать, слой считает долю событий, которые выглядят как настоящий браузер.
Событие засчитывается как настоящий браузер, только если совпало все сразу: UA браузерный,
число шифров браузерное, и хеши отпечатка не из словаря инструментов. Если доля таких
событий выше порога — блокировать нельзя при любой оценке: под сигнатурой ходят живые
люди. Три условия вместе потому, что UA подделать легко, а почерк рукопожатия — труднее:
«UA = Chrome, почерк = curl» считается маскировкой, а не человеком.
