...
**бот-онли** (вето). Считаем долю запросов под сигнатурой, похожих на живой браузер, и
блокируем только если их почти нет.

1. Взять все запросы сигнатуры за окно.
2. Пометить запрос «настоящий браузер», если выполнено всё сразу: (а) в `User-Agent` назван
браузер (Chrome/Firefox/Safari…), а не curl/python/бот; (б) число шифров в отпечатке
браузерное (15/16/20); (в) хеши отпечатка не из словаря инструментов. Три условия вместе —
потому что UA подделать легко, а почерк рукопожатия трудно: «UA = Chrome, а почерк = curl»
это маскировка, а не человек.
3. Доля = помеченные / все запросы (например, 30 из 1000 = 3%).
4. Пройден, если доля ≤ 5%; выше — вето: под сигнатурой есть живые пользователи, блокировать
нельзя ни при какой оценке.

Условия (б) и (в) — свойства самого отпечатка, для всех его запросов одинаковые; меняется
только `User-Agent`. Поэтому доля фактически показывает, как часто под этим рукопожатием
приходили с настоящим браузерным UA.

**объем.** Проверяем, что данных накопилось достаточно для решения.

1. Взять накопленный за все время счетчик событий сигнатуры (из накопителя) и число ее IP за окно.
2. Пройден, если событий ≥ 20 И есть хотя бы один IP; иначе придержать. Это мягкий гейт: по
паре случайных запросов решение не принимают.
...
3. Пройден, если обоснованность есть; иначе автоблока нет. Общий отпечаток curl/python трогать
нельзя (заденет легитимных) — это случай для адресного UA/IP, решает человек.

**known-good** (вето). Сверяем отпечаток с позитивным каталогом настоящих браузеров. Пройден,
если совпадения нет; совпал — вето: это настоящий Chrome/Firefox, его не блокируют никогда
(см. «Защита от отравления»).
...
`active` и с выключенным attack mode (см. оговорку ниже).
2. При достаточном числе выданных посчитать долю решенных.
3. Пройден, если доля низкая (челленджи почти не решаются — так ведет себя бот). Высокая доля
— вето: челленджи проходят, значит под отпечатком живые люди. Низкая доля вдобавок ускоряет
переход `наблюдение → блокировка`.
