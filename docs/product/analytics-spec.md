...
2. Пометить запрос «настоящий браузер», если выполнено всё сразу: (а) в `User-Agent` назван браузер (Chrome/Firefox/Safari…), а не curl/python/бот; (б) число шифров в отпечатке браузерное (15/16/20); (в) хеши отпечатка не из словаря инструментов (`tls_fp_catalog`).
Три условия вместе — потому что UA подделать легко, а почерк рукопожатия трудно: «UA = Chrome, а почерк = curl» это маскировка, а не человек.
3. Доля = помеченные запросы / все запросы 
4. Пройден, если доля ≤ 5%; выше — вето: так как под сигнатурой есть живые пользователи, блокировать  
...
Откуда под сигнатурой вообще живые люди: отпечаток задается TLS-стеком, а не личностью.
Если бот ходит с тем же стеком, что и реальный (возможно, редкий) браузер, под одним fp
смешиваются бот и живые пользователи этого браузера — гейт стережет именно этот случай. Под
инструментальным fp (curl/python) людей нет вовсе, там доля = 0 и этот гейт не помогает — его
страхует «обоснованность».
...
#### Когда автомат готовит черновик PR

Результат прогона для сигнатуры — не блокировка, а запись в черновике PR (и то на
наблюдение). Автомат кладет туда кандидата, только если совпало все: тир HIGH, сигнатура
видна достаточно дней, все гейты пройдены и есть обоснованность. Остальное (HIGH без
обоснованности, MEDIUM, LOW) автомат не трогает — оно остается в отчете на усмотрение
человека.
...
условие из «Когда автомат готовит черновик PR»). Это пока прогноз: вывод сделан
