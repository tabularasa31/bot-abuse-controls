...
| **позитивный каталог браузеров** (`tls_fp_browser_profiles`) | белый список «известно-хороших» браузеров. Сегодня грубо: `семейство → ожидаемое число шифров` (кормит suspicious cipher S2). Улучшение D13 — обогатить тот же каталог до `полный fp → семейство/версия` (не новый каталог): тогда genuine-browser/«бот-онли» (G1), known-good (G6), version mismatch (S9) и инвертированный `staging` (запись = «уже доверяем»). `cipher_cnt` и так часть полного fp. |
