...
`tls_fp_browser_known` и `tls_fp_catalog` наполняет харвестер отпечатков (см. ниже),
`tls_fp_browser_profiles` читается с тех же браузеров; хеши не пишут руками. Сам слой
...
| **харвестер отпечатков** | CI cron-one-shot: гонит реальные браузеры (Chrome/FF/Edge) и инструменты (curl/python/go/okhttp/headless) на `/__fp` → полный fp → draft-PR в `tls_fp_browser_known` (браузеры) и `tls_fp_catalog` (инструменты). Каденс по дрейфу TLS (раз в несколько мажоров). Мерж — человек; края (LibreWolf/Tor/mobile/экзотика) и незнакомые отпечатки из логов — ручной PR. |
