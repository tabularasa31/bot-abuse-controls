...
| Сущность | Определение |
| --- | --- |
| **gate (гейт)** | порог допуска к блокировке: про безопасность блока (не заденет ли невиновных под тем же fp), а не про то, бот ли это. Гейты НЕ складываются и НЕ компенсируют друг друга. Вето (бот-онли/доверенные/known-good/решение челленджей) охраняют живых людей и легитимный трафик — не пройден → не блокируем, без исключений. Объём/дубль — про качество данных: лишь придерживают авто-промоут, человек на ревью может пропустить. |
