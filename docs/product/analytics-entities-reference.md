...
| Сущность | Определение |
| --- | --- |
| **gate (гейт)** | порог допуска к блокировке: про безопасность блока (не заденет ли невиновных под тем же fp), а не про то, бот ли это. Гейты НЕ складываются и НЕ компенсируют друг друга. Автомат не перебивает ни один; вручную (инструмент промоута) у мягкого гейта (объём) есть точечный оверрайд, у жёстких вето (бот-онли/доверенные/known-good/решение челленджей) штатного оверрайда нет — обойти можно только грубым промоутом без проверки гейтов, под ответственность оператора. |
...
Отдельные от потока логов справочники, на которые опираются сигналы и гейты. Позитивный
каталог браузеров наполняет браузерная ферма (см. ниже), остальные курируемые (продакт
через PR, ADR-006); слой их только читает. Сам слой наполняет (draft-PR) лишь блок-лист
`tls_fp_blocklist`.
...
| **позитивный каталог браузеров** (`tls_fp_browser_known`) | белый список «известно-хороших»: полный fp реального браузера → `{семейство, статус active\|staging}`. Дает genuine-browser проверку (гейт «бот-онли»), known-good (G6), version mismatch (S9). Наполняет браузерная ферма. У `staging` **инвертированная** семантика vs blocklist: запись уже доверенная, гасит флаг. |
...
| **браузерная ферма (харвестер)** | CI cron-one-shot: Playwright гонит реальные stable Chrome/FF/Edge на `/__fp` → полный fp → draft-PR в `tls_fp_browser_known`. Каденс по дрейфу TLS (раз в несколько мажоров). Мерж — человек; края (LibreWolf/Tor/mobile) — ручной PR. |
