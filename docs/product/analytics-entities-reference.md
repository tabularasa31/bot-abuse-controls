...
| **настоящий браузер** | сигнатура, чей fp есть в каталоге настоящих браузеров (`tls_fp_browser_profiles`). В модели полного fp это единственный признак «под отпечатком живые люди» — отпечаток задаётся стеком, блокировка идёт по точному fp, поэтому отдельной «доли живых» (старый purity/human_share) не нужно. |
...
| **known-good (вето)** | гейт: fp есть в каталоге настоящих браузеров → **никогда** не блокировать (реальный браузер, под ним живые люди). Единственная проверка «не живой ли это браузер» — заменяет старый purity-гейт по доле. |
...
| **позитивный каталог браузеров** (`tls_fp_browser_profiles`) | белый список «известно-хороших» браузеров. Сегодня грубо: `семейство → ожидаемое число шифров` (кормит suspicious cipher S2). Планируемое улучшение — обогатить тот же каталог до `полный fp → семейство/версия` (не новый каталог): тогда known-good / настоящий браузер (G5), version mismatch (S9) и инвертированный `staging` (запись = «уже доверяем»). `cipher_cnt` и так часть полного fp. |
