...
Документ описывает целевое поведение WAF-слоя как отдельный слой поверх каскада
([vision.md](vision.md)): спецификация «бери и делай». Выбор движка (build-vs-buy) —
сознательно открытый вопрос, решается спайком и архитектурным решением (см. §4); до
этого решения код не коммитим.
