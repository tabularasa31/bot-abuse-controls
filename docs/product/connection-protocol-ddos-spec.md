...
Это третий слой DDoS-картины (см. roadmap §4): L7 rate-based уже закрыт G-серией
(G1–G5), волюметрика L3/L4 — вне нашего скоупа (reality-3). Этот документ — про
...
reputation   →  ЭСКАЛИРУЕТ (G2 подсеть/IP → G3 → edge-ACL)   ← общий сток DDoS
```
...
`bac_log` + теги, `metrics.lua`, G2 (subnet/IP reputation) как сток повторных нарушителей,
