...
--   license: Apache-2.0
--
-- WHY vendored: openresty/openresty:alpine does not bundle this library and
-- the demo stand runs the stock image with no build step (Lua is bind-mounted,
-- same approach as ja4_compute). Dropping the single self-contained file here
-- keeps "edit + restart" working without a Dockerfile/opm install. All of its
-- requires (resty.core.base, bit, table.clear, ffi, ngx.errlog) ARE part of
-- the OpenResty bundle. To update: replace this file with a newer tagged
-- release and bump the version line above.
-- luacheck: ignore
