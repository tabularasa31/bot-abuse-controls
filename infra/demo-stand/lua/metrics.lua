...
# HELP antibot_clearance_verify_total L2.1 clearance cookie verify outcomes (C3). valid = HMAC ok + matching site + not expired (fastpath fired, L3/L5 skipped). invalid = HMAC mismatch or compute failure (incl. challenge_secret not loaded — see WARN log). expired = HMAC ok but exp <= now. missing = no cookie header. malformed = cookie structure unparseable. wrong_site = HMAC ok but cookie's bound site != request host (cross-tenant leak attempt).
